...
.search-icon-employee-swipes::before {
    ...
    <div class="button-container">
       ...
       <a href="{{ route('add-damage-page') }}" class="add-employee-button"><i class="fa-sharp fa-solid fa-plus"></i>Add Damage</a>
    ...
    <div style="margin-top:40px;display:flex;flex-direction:row;gap:5px;margin-left:15px;">
        ...
        <div class="dropdown-for-employee">
            <button class="dropdown-button-for-employee"wire:click="togglePicker"><span style="font-size:12px;">Installments: {{ ucwords($selectedOption) }}</span> <span class="arrow-for-employee"></span></button>
            @if($showPicker==true)
                  <div class="dropdown-content-for-employee">
      ...
      <table id="employee-table">
        ...
        <tbody>
           @php
             $i=0;
           @endphp
            @if (count($damageDetails)>0)
                    @foreach ($damageDetails as $dad)
                    
                    
                    <tr>
                        <td style="font-size:12px;">{{ ++$i }}.</td>
                        <td style="font-size:12px;">{{ \Carbon\Carbon::parse($dad->damage_or_loss_date)->format('jS F,Y') }}</td>
                        <td style="text-decoration:underline;font-size:12px;">{{ $dad->emp_id }}</td>
                        <td style="white-space:nowrap;font-size:12px;">{{ $dad->name_of_the_person }}</td>
                        <td style="font-size:12px;">
                            {{ $dad->damage_or_loss_reason }}
                        </td>
                        <td style="font-size:12px;">  {{ \App\Helpers\FormatHelper::formatAmount($dad->amount_of_damage) }}</td>
                        <td style="font-size:12px;">{{ $dad->no_of_installments }}</td>
                        <td style="font-size:12px;">{{ \Carbon\Carbon::parse($dad->recovery_first_installment_date)->format('jS F,Y') }}</td>
                        <td style="font-size:12px;">{{ \Carbon\Carbon::parse($dad->recovery_last_installment_date)->format('jS F,Y') }}</td>
                        <td style="font-size:12px;">{{ $dad->damage_or_loss_reason }}</td>
                        <td style="font-size:12px;">

                            <a href="{{ route('add-damage-page', ['id' => $dad->id, 'viewMode' => true]) }}" class="edit-employee-button">
                                    <i class="fa-sharp fa-solid fa-eye"></i>
                                </a>
                                <a href="{{ route('add-damage-page', ['id' => $dad->id]) }}" class="edit-employee-button">
                                    <i class="fa-sharp fa-solid fa-edit"></i>
                                </a>
                                <a href="#"class="text-danger"wire:click="openDeleteModal({{  $dad->id  }})"title="Delete">
