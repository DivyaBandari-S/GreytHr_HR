<div class="container mt-4">
        ...
        @if ($showHelp == false)
            <div class="row main-overview-help">
                <div class="col-md-11 col-10 d-flex flex-column">
                    <p class="main-overview-text">The Generate Letter wizard guides you through the process of generating a letter for an employee. You can also send it to one or multiple employees in one effort. Note: You can download a copy of the letter in the Generate Letter: Summary Page.</p>
                    <p class="main-overview-text">Learn more about the process by watching the  <span class="main-overview-highlited-text">
                        video</span> here. To view frequently asked questions <span class="main-overview-highlited-text"> click</span>
                        here.</p>
                </div>
    ...
    <div class="progress-container">
        ...
        <!-- Step 3 -->
        <div class="progress-step">
            <div class="circle {{ $currentStep >= 3 ? ($currentStep == 3 ? 'active' : 'completed') : 'pending' }}">3</div>
            <div class="label">Preview</div>
        </div>
    </div>

    <div class="mt-4 p-4 border rounded bg-white row">
        <div class="col-md-6">
            @if($currentStep == 1)
            ...
            <div class="mb-3">
                ...
                <select class="form-select" wire:model="authorized_signatory">
                    <option value="Daisy M (CEO)">Daisy M (CEO)</option>
                    <option value="John D (HR Manager)">John D (HR Manager)</option>
                </select>
                @error('authorized_signatory') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            ...
            <div class="mb-3 d-flex align-items-center">
                ...
                <div class="form-check form-check-inline">
                    <input type="radio" id="singleEmployee" class="form-check-input" selected value="single">
                    <label class="form-check-label" for="singleEmployee">Single</label>
                </div>
                <div class="form-check form-check-inline">
                    <input type="radio" id="multipleEmployees" class="form-check-input" value="multiple">
                    <label class="form-check-label" for="multipleEmployees">Multiple</label>
                </div>
                @error('generateFor') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
...
@enderror
            ...
            {{-- CTC Field --}}
            <div class="mb-3">
                <label for="ctc" class="fw-bold">CTC</label>
                <input type="text" id="ctc" class="form-control" placeholder="Enter CTC..." >
                @error('ctc') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            ...
            @if($previewLetter)
                <div class="border p-3 mt-3">
                    <h4 class="text-center">{{ $previewLetter['template_name'] }}</h4>
                    <p><strong>Date:</strong> {{ now()->format('d M Y') }}</p>
                    <p><strong>To:</strong> {{ $previewLetter['employee_name'] }}</p>
                    <p><strong>Employee ID:</strong> {{ $previewLetter['employee_id'] }}</p>
                    <p><strong>Address:</strong> {{ $previewLetter['employee_address'] }}</p>
                    <div class="mt-4">
                        {!! $previewLetter['letter_content'] !!}
                    </div>
                </div>
        
                {{-- <button class="btn btn-success mt-3" >Save Letter</button> --}}
            @else
